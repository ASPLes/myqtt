...
def test_17c_common (label, topic, msg_content):
    # call to initialize a context 
    ctx = myqtt.Ctx ()
    ...
    info ("Test %s: checking big message (checking with message size: %d).." % (label, len (msg_content)))
    ...
    # subscripbe
    (status, sub_qos) = conn.sub (topic, myqtt.qos0)
    if not status:
        error ("Failed to subscribe..")
        return False
    ...
    if not conn.pub (topic, msg_content, len (msg_content), myqtt.qos0, False, 10):
        error ("Unable to send message to get current connecting user..")
        return False
...
def test_17c ():
    if not test_17c_common ("17-c", "this/is/a/test", define_big_mesg):
        return False

    if not test_17c_common ("17-c1", "this/is/a/test/this-is-a-very-long-topicthis/is/a/test/this-is-a-very-long-topic", define_big_mesg):
        return False
    ...
    if not test_17c_common ("17-c4", "this/is/a/test", define_big_mesg_2):
        return False

    if not test_17c_common ("17-c5", "this/is/a/test/this-is-a-very-long-topicthis/is/a/test/this-is-a-very-long-topic", define_big_mesg_2):
        return False
