 ...
 * 
 * Initializes the myqtt thread pool. This pool is mainly used to
 * invoke frame receive handler at any level. Because we have always
 * running threads to send messages (the myqtt writer and sequencer)
 * this pool is not needed. 
 *
 * Among the myqtt features are listed to create connection, channels
