...
#define LOG_DOMAIN "myqtt-support"
...
void myqtt_support_init (MyQttCtx * ctx)
{
	...
	/* init hte mutex */
	myqtt_mutex_create (&ctx->search_path_mutex);

	/* init inet_ntoa mutex */
	myqtt_mutex_create (&ctx->inet_ntoa_mutex);

	return;
}

/** 
 * @brief Allows to cleanup the myqtt support module state from the
 * provided \ref MyQttCtx object.
 * 
 * @param ctx The myqtt context to clean up (only module specific
 * part).
 */
void myqtt_support_cleanup (MyQttCtx * ctx)
{
	...
	/* destroy mutex */
	myqtt_mutex_destroy (&ctx->search_path_mutex);

	/* terminate inet_ntoa mutex */
	myqtt_mutex_destroy (&ctx->inet_ntoa_mutex);

	return;
}

/**
 * \defgroup myqtt_support MyQtt Support: Support function used across the library
 */
...
/** 
 * @brief Allows to add new search path to be used by \ref myqtt_support_find_data_file.
 *
 * This function allows to configure how MyQtt Library lookups
 ...
 * myqtt_support_add_search_path (ctx, "/my/path/to/local/data");
 * myqtt_support_add_search_path (ctx, "/my/alternative/path");
 *
 * // where ctx is an initialized MyQttCtx object
 *
 * ...
 *
 * // write you file location code in an abstract manner so it could be
 * // used on every platform easily.
 * char  * my_data_def = myqtt_support_find_data_file (ctx, "my_data.def");
 *
 ...
 * myqtt_support_add_domain_search_path.
 *
					 ...
					 const char  * path)
{
	...
	myqtt_support_add_domain_search_path (ctx, "default",  path);
	return;
}
						    ...
						    const char * path)
{
	...
	myqtt_mutex_lock (&ctx->search_path_mutex);
	...
	while (iterator < axl_list_length (ctx->support_search_path)) {
		...
		if (axl_cmp (node->domain, domain) && axl_cmp (node->path, path)) {
			...
			myqtt_mutex_unlock (&ctx->search_path_mutex);	
			return axl_true;
		} /* end if */
 ...
 * making a local copy.
 *
 * This function works like \ref myqtt_support_add_search_path
 ...
 *
 * If a static path is needed to be added \ref myqtt_support_add_search_path should be used.
 *
 * @param ctx The context where the operation will be performed.
 *
					     ...
					     char      * path)
{
	...
	if (myqtt_support_check_search_path (ctx, "default", path)) {
	...
	myqtt_support_add_domain_search_path_ref (ctx, axl_strdup ("default"), path);
 ...
 * myqtt_support_add_domain_search_path_ref, allow to configure the
 * search function \ref myqtt_support_domain_find_data_file with new
 * search path included inside a domain.
 *
 * This function works like \ref myqtt_support_add_search_path but
 ...
 * myqtt_support_domain_find_data_file.
 */
						    ...
						    const char * path)
{
	...
	/* check if the path already exists */
	if (myqtt_support_check_search_path (ctx, domain, path))
		return; /* do not added it, already added */

	/* call to default implementation */
	myqtt_support_add_domain_search_path_ref (ctx, 
						   axl_strdup (domain),
						   axl_strdup (path));
	return;	
}
...
/** 
 * @brief Allows to lookup for a file into the known myqtt data file
 * locations.
 * 
 * While locating data files inside MyQtt Library context a set of
 ...
 *
 * See \ref myqtt_support_add_search_path for more information about
 ...
 * searches using \ref myqtt_support_domain_find_data_file and its
 * associated function to install search path with a domain constraing
 * them:
 *
 *   - \ref myqtt_support_add_domain_search_path
 *   - \ref myqtt_support_add_domain_search_path_ref
 *
 * @param ctx The context where the operation will be performed.
...
char   * myqtt_support_find_data_file (MyQttCtx   * ctx, 
					const char  * name)
{
	return myqtt_support_domain_find_data_file (ctx, "default", name);
}
						    ...
						    const char * name)
{
	...
	while (axl_list_cursor_has_item (cursor)) {
		...
		if (axl_cmp (node->domain, domain)) {
			...
			if (axl_cmp (node->path, "."))
				...
				file_name = axl_strdup_printf ("%s%s%s", node->path, MYQTT_FILE_SEPARATOR, name);

			/* check the file to be found */
			if (myqtt_support_file_test (file_name, FILE_EXISTS | FILE_IS_REGULAR)) {
				/* free the cursor */
				axl_list_cursor_free (cursor);

				myqtt_log (MYQTT_LEVEL_DEBUG, "file found at: %s, %d, %p", file_name, 
					    axl_list_length (ctx->support_search_path), ctx->support_search_path);
				myqtt_mutex_unlock (&ctx->search_path_mutex);	
				return file_name;
			} /* end if */
			
			myqtt_log (MYQTT_LEVEL_DEBUG, "unable to find file %s on '%s', %d, %p",  
	...
	myqtt_log (MYQTT_LEVEL_DEBUG, "unable to find %s inside domain: '%s', %d, %p", name, domain,
		    axl_list_length (ctx->support_search_path), ctx->support_search_path);
 ...
 * provide must be not NULL. To unset an environment variable use \ref myqtt_support_unsetenv
 *
...
char   * myqtt_support_build_filename      (const char * name, ...)
{
	...
	while (token != NULL) {
		aux    = result;
		result = axl_strdup_printf ("%s%s%s", result, MYQTT_FILE_SEPARATOR, token);
		axl_free (aux);
 ...
 *
 * @param ctx The myqtt context where the operation will be
						    ...
						    struct sockaddr_in  * sin)
{
	...
	myqtt_mutex_lock (&ctx->inet_ntoa_mutex);

	/* allocate the string */
	result = axl_strdup (inet_ntoa (sin->sin_addr));

	/* unlock */
	myqtt_mutex_unlock (&ctx->inet_ntoa_mutex);
...
/*	BOOL                    unit      = axl_true; */
	...
	if ((listener_fd = socket(AF_INET, SOCK_STREAM, 0)) <= 2) {
		/* do not allow creating sockets reusing stdin (0),
		   stdout (1), stderr (2) */
		myqtt_log (MYQTT_LEVEL_CRITICAL, "failed to create listener socket: %d (errno=%d:%s)", listener_fd, errno, myqtt_errno_get_error (errno));
		return -1;
        } /* end if */
	...
	if (bind_res == MYQTT_SOCKET_ERROR) {
		myqtt_log (MYQTT_LEVEL_CRITICAL, "unable to bind address (port already in use or insufficient permissions). Closing socket: %d", listener_fd);
		myqtt_close_socket (listener_fd);
		return -1;
	}
	
	if (listen (listener_fd, 1) == MYQTT_SOCKET_ERROR) {
		myqtt_log (MYQTT_LEVEL_CRITICAL, "an error have occur while executing listen");
		myqtt_close_socket (listener_fd);
		return -1;
        } /* end if */
	...
	if (getsockname (listener_fd, (struct sockaddr *) &sin, &sin_size) < -1) {
		myqtt_log (MYQTT_LEVEL_CRITICAL, "an error have happen while executing getsockname");
		myqtt_close_socket (listener_fd);
		return -1;
	} /* end if */

	myqtt_log  (MYQTT_LEVEL_DEBUG, "created listener running listener at %s:%d (socket: %d)", inet_ntoa(sin.sin_addr), ntohs (sin.sin_port), listener_fd);

	/* on now connect: read side */
	descf[0]      = socket (AF_INET, SOCK_STREAM, 0);
	if (descf[0] == MYQTT_INVALID_SOCKET) {
		myqtt_log (MYQTT_LEVEL_CRITICAL, "Unable to create socket required for pipe");
		myqtt_close_socket (listener_fd);
		return -1;
	} /* end if */

	/* disable nagle */
	myqtt_conn_set_sock_tcp_nodelay (descf[0], axl_true);

	/* set non blocking connection */
	myqtt_conn_set_sock_block (descf[0], axl_false);  
	...
	if (result < 0 && errno != MYQTT_EINPROGRESS) {
		myqtt_log (MYQTT_LEVEL_CRITICAL, "connect () returned %d, errno=%d:%s", 
			    result, errno, myqtt_errno_get_last_error ());
		myqtt_close_socket (listener_fd);
		return -1;
	}

	/* accept connection */
	myqtt_log  (MYQTT_LEVEL_DEBUG, "calling to accept () socket");
	descf[1] = myqtt_listener_accept (listener_fd);

	if (descf[1] <= 0) {
		myqtt_log (MYQTT_LEVEL_CRITICAL, "Unable to accept connection, failed to create pipe");
		myqtt_close_socket (listener_fd);
		return -1;
	}
	/* set pipe read end from result returned by thread */
	myqtt_log (MYQTT_LEVEL_DEBUG, "Created pipe [%d, %d]", descf[0], descf[1]);

	/* disable nagle */
	myqtt_conn_set_sock_tcp_nodelay (descf[1], axl_true);
