...
#define LOG_DOMAIN "myqtt-hash"

/** 
 * \defgroup myqtt_hash MyQttHash: Thread Safe Hash table used inside MyQtt Library.
 */

/** 
 * \addtogroup myqtt_hash
 * @{
 */
 ...
 */
void __myqtt_hash_notify_change (MyQttHash * hash_table)
{
	int waiters;
	...
	waiters = myqtt_async_queue_waiters (hash_table->changed_queue);
	while (waiters > 0) {

		/* push queue one data for each waiter */
		myqtt_async_queue_push (hash_table->changed_queue, INT_TO_PTR (1));
...
/** 
 * @brief Creates a new MyQttHash setting all functions.
 * 
 * Creates a new MyQtt Hash Table. All myqtt library is programed
 ...
 * allow myqtt library to create critical section to all operation
 ...
 *
 * @return a new MyQttHash table or NULL if fail
 **/
...
/** 
 * @brief Creates a new MyQttHash without providing destroy function.
 * 
 * A myqtt_hash_new_full version passing in as NULL key_destroy_func and 
 * value_destroy_func. 
 *
 * @param hash_func
 * @param key_equal_func
 * 
 * @return a newly created MyQttHash or NULL if fails.
 **/
MyQttHash * myqtt_hash_new      (axlHashFunc    hash_func,
				   axlEqualFunc   key_equal_func)
{
	return myqtt_hash_new_full (hash_func, key_equal_func, NULL, NULL);
}

/** 
 * @brief Allows to increase in one unit the reference counting on the
 * hash table received. A call to \ref myqtt_hash_unref will be
 * required to reduce the reference counting. Reaching 0 will cause
 * \ref myqtt_hash_destroy to be called automatically.
 *
...
void         myqtt_hash_ref      (MyQttHash   * hash_table)
{
	...
	myqtt_mutex_lock (&hash_table->mutex);
	hash_table->ref_count++;
	myqtt_mutex_unlock (&hash_table->mutex);
	return;
}

/**
 * @brief Decrease reference counting and, if reached 0 reference a
 * call to \ref myqtt_hash_destroy is done.
 *
 * @param hash_table
 */
void         myqtt_hash_unref    (MyQttHash   * hash_table)
{
	 ...
	 * myqtt_hash_destroy already implements reference
	 * counting  */
	myqtt_hash_destroy (hash_table);
} 

/**
 * @brief Inserts a pair key/value inside the given MyQttHash
 *
 * 
				   ...
				   axlPointer value)
{
	...
	/* notify change */
	__myqtt_hash_notify_change (hash_table);
					     ...
					     long          wait_microseconds)
{
	...
	if (hash_table->changed_queue == NULL)
		hash_table->changed_queue = myqtt_async_queue_new ();
	...
	if (wait_microseconds > 0)
		result = PTR_TO_INT (myqtt_async_queue_timedpop (hash_table->changed_queue, wait_microseconds));
	else 
		result = PTR_TO_INT (myqtt_async_queue_pop (hash_table->changed_queue));

	/* reduce reference counting */
	myqtt_hash_unref (hash_table);
...
void         myqtt_hash_destroy  (MyQttHash *hash_table)
{
	...
	hash_table->table = NULL;
 	...
 	if (hash_table->changed_queue)
 		myqtt_async_queue_unref (hash_table->changed_queue);

	/* unlock and free */
	myqtt_mutex_unlock (&hash_table->mutex);
	myqtt_mutex_destroy (&hash_table->mutex);
...
/** 
 * @brief Perform a foreach over all elements inside the MyQttHash.
 * 
...
/** 
 * @internal
 * @brief Support function for \ref myqtt_hash_clear function.
 * 
 * It just returns axl_true.
 */
axl_bool      myqtt_hash_clear_allways_true (axlPointer key, axlPointer value, axlPointer user_data) 
{
	return axl_true;
}
