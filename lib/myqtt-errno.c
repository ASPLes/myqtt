...
void    myqtt_errno_show_error        (MyQttCtx * ctx, int  __errno)
{

#if defined(AXL_OS_WIN32)
	switch (errno) {
	case WSAEINTR:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEINTR);
	case WSAEACCES:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEACCES);
	case WSAEFAULT:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEFAULT);
	case WSAEINVAL:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEINVAL);
	case WSAEMFILE:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEFAULT);
	case WSAEWOULDBLOCK:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEWOULDBLOCK);
	case WSAEINPROGRESS:        
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEINPROGRESS);
	case WSAEALREADY:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEALREADY);           
	case WSAENOTSOCK:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAENOTSOCK);
	case WSAEDESTADDRREQ:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEDESTADDRREQ);
	case WSAEMSGSIZE:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEMSGSIZE);           
	case WSAEPROTOTYPE:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEPROTOTYPE);
	case WSAENOPROTOOPT:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAENOPROTOOPT);
	case WSAEPROTONOSUPPORT:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEPROTONOSUPPORT);
	case WSAESOCKTNOSUPPORT:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAESOCKTNOSUPPORT);
	case WSAEOPNOTSUPP:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEOPNOTSUPP);
	case WSAEPFNOSUPPORT:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEPFNOSUPPORT);
	case WSAEAFNOSUPPORT:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEAFNOSUPPORT);
	case WSAEADDRINUSE:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEADDRINUSE);
	case WSAEADDRNOTAVAIL:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEADDRNOTAVAIL);
	case WSAENETDOWN:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAENETDOWN);
	case WSAENETUNREACH:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAENETUNREACH);
	case WSAENETRESET:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAENETRESET);
	case WSAECONNABORTED:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAECONNABORTED);
	case WSAECONNRESET:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAECONNRESET);
	case WSAENOBUFS:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAENOBUFS);
	case WSAEISCONN:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEISCONN);
	case WSAENOTCONN:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAENOTCONN);
	case WSAESHUTDOWN:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAESHUTDOWN);
	case WSAETIMEDOUT:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAETIMEDOUT);
	case WSAECONNREFUSED:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAECONNREFUSED);
	case WSAEHOSTDOWN:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEHOSTDOWN);
	case WSAEHOSTUNREACH:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEHOSTUNREACH);
	case WSAEPROCLIM:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEPROCLIM);
	case WSASYSNOTREADY:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSASYSNOTREADY);
	case WSAVERNOTSUPPORTED:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAVERNOTSUPPORTED);
	case WSANOTINITIALISED:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSANOTINITIALISED);
	case WSAEDISCON:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAEDISCON);
	case WSATYPE_NOT_FOUND:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSATYPE_NOT_FOUND);     
	case WSAHOST_NOT_FOUND:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSAHOST_NOT_FOUND);
	case WSATRY_AGAIN:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSATRY_AGAIN);
	case WSANO_RECOVERY:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSANO_RECOVERY);
	case WSANO_DATA:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSANO_DATA);            
	case WSA_INVALID_HANDLE:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSA_INVALID_HANDLE);
	case WSA_INVALID_PARAMETER:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSA_INVALID_PARAMETER);
	case WSA_IO_INCOMPLETE:     
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSA_IO_INCOMPLETE);
	case WSA_IO_PENDING:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSA_IO_PENDING);
	case WSA_NOT_ENOUGH_MEMORY:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSA_NOT_ENOUGH_MEMORY);
	case WSA_OPERATION_ABORTED:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSA_OPERATION_ABORTED);
	case WSASYSCALLFAILURE:
		myqtt_log (MYQTT_LEVEL_CRITICAL,  ERRNO_WSASYSCALLFAILURE);
	}
#elif defined (AXL_OS_UNIX)
	/* show the last error */
	myqtt_log (MYQTT_LEVEL_CRITICAL, strerror (errno));
#endif
	return;
}

void    myqtt_errno_show_last_error   (MyQttCtx * ctx)
{
	myqtt_errno_show_error (ctx, errno);

	return;
}

char  * myqtt_errno_get_error         (int  __errno)
{
#if defined(AXL_OS_WIN32)
	switch (errno) {
	case WSAEINTR:
...
char  * myqtt_errno_get_last_error    (void)
{
