...
int  myqtt_win32_init (MyQttCtx * ctx)
{
	...
	if (error != NO_ERROR) {
		myqtt_log (MYQTT_LEVEL_CRITICAL, "unable to init winsock api, exiting..");
		return axl_false;
	}
	myqtt_log (MYQTT_LEVEL_DEBUG, "winsock initialization ok");
	return axl_true;
}
                       ...
                       LPVOID reserved)
{
    ...
    /* always returns true because myqtt init is done through
     * myqtt_init */
    return axl_true;
}

int      __myqtt_win32_blocking_socket_set (MYQTT_SOCKET socket,
                                             int           status) 
{
...
{
        return __myqtt_win32_blocking_socket_set (socket, 1);
}

int      myqtt_win32_blocking_enable (MYQTT_SOCKET socket)
{
        return __myqtt_win32_blocking_socket_set (socket, 0);
}
